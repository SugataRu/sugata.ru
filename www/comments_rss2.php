...
if ($_REQUEST['q']) {
    ...
    $title = $globals['site_name'].': '._('поиск в комментариях') . ': ' . htmlspecialchars(strip_tags($_REQUEST['q']));
    ...
    $title = $globals['site_name'].': '._('комментарии') . " [$id]";
    $globals['redirect_feedburner'] = false;
    ...
    $title = $globals['site_name'].': '.sprintf(_('комментарии от %s'), $username);
    $globals['redirect_feedburner'] = false;
    ...
    $title = $globals['site_name'].': '.sprintf(_('комментарии новости от %s'), $username);
    $globals['redirect_feedburner'] = false;
    ...
    $title = $globals['site_name'].': '.sprintf(_('ответы на %s'), $username);
    $globals['redirect_feedburner'] = false;
} else {
    //
    // All comments
    ...
    $title = $globals['site_name'].': '._('комментарии');
    $globals['redirect_feedburner'] = false;
}
...
if ($comments) {
    foreach ($comments as $comment_id) {
        ...
        if ($comment->user_level == 'disabled') {
            $content = '['._('Пользователь отключен').']';
        } else {
            $content = htmlentities2unicodeentities($comment->to_html($comment->content));
        }
        echo "	<item>\n";
        echo "		<sugata:comment_id>$comment->id</sugata:comment_id>\n";
        echo "		<sugata:link_id>$comment->link</sugata:link_id>\n";
        echo "		<sugata:order>$comment->order</sugata:order>\n";
        echo "		<sugata:user>$comment->username</sugata:user>\n";
        echo "		<sugata:votes>".intval($comment->votes)."</sugata:votes>\n";
        echo "		<sugata:karma>".intval($comment->karma)."</sugata:karma>\n";
        echo "		<sugata:url>".$comment->link_permalink."</sugata:url>\n";
...
function do_header($title)
{
    ...
    echo"	<image><title>".get_server_name()."</title><link>http://".get_server_name()."</link><url>http://".get_static_server_name().$globals['base_url']."img/eli-rss.png</url></image>\n";
    echo'	<description>'._('Совместный сайт для публикации и общения ').'</description>'."\n";
    echo'	<pubDate>'.date("r", $last_modified).'</pubDate>'."\n";
    echo'	<generator>http://sugata.ru/</generator>'."\n";
