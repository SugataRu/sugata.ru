...
if (empty($_REQUEST['id']) && $url_args[0] && !ctype_digit($url_args[0])) {
    ...
    if (!$link) {
        do_error(_('новости не найдены'), 404);
    }
} else {
...
if (($argc = count($url_args)) > 1) {
    ...
    if (preg_match('/^c(\d+)$/', $url_args[1], $comment)) {
        ...
        if ($current_page > $total_pages) {
            do_error(_('несуществующая страница'), 404);
        }
        if ($argc == 2) {
...
$globals['extra_head'] .= '<link rel="alternate" type="application/rss+xml" title="'._('комментирует эту новость').'" href="'.$globals['scheme'].'//'.get_server_name().$globals['base_url'].'comments_rss?id='.$link->id.'" />';
...
if (!empty($new_comment_error)) {
    add_javascript('mDialog.notify("'._('предупреждение').": $new_comment_error".'", 5);');
}
do_tabs('main', _('новости'), true);
if (
    empty($link->url)
    && (
...
function print_story_tabs($option)
{
    ...
    $html .= '<option value="'.$globals['permalink'].'/standard">'._('по времени').'</option>';
    $html .= '<option value="'.$globals['permalink'].'/threads" '.$active[10].'>'._('древовидные').'</option>';
    $html .= '<option value="'.$globals['permalink'].'/best-threads" '.$active[11].'>'._('лучшие темы').'</option>';
    $html .= '<option value="'.$globals['permalink'].'/best-comments" '.$active[2].'>'._('+ номинальные').'</option>';
    if (!$globals['bot']) {
        // Don't show "empty" pages to bots, Google can penalize too
        if ($globals['link']->sent_date > $globals['now'] - 86400 * 60) { // newer than 60 days
            $html .= '<option value="'.$globals['permalink'].'/voters" '.$active[3].'>'._('по голосам').'</option>';
        }
        if ($globals['link']->sent_date > $globals['now'] - 86400 * 30) {
            // newer than 30 days
            $html .= '<option value="'.$globals['permalink'].'/log" '.$active[4].'>'._('логи').'</option>';
        }
        if ($globals['link']->date > $globals['now'] - $globals['time_enabled_comments']) {
            $html .= '<option value="'.$globals['permalink'].'/sneak" '.$active[5].'>&micro;&nbsp;'._('активности').'</option>';
        }
    }
    if ($current_user->user_id > 0) {
        if (($c = $db->get_var("SELECT count(*) FROM favorites WHERE favorite_type = 'link' and favorite_link_id=$link->id")) > 0) {
            $html .= '<option value="'.$globals['permalink'].'/favorites" '.$active[6].'>'._('фавориты')."&nbsp;($c)</option>";
        }
    }
    $html .= '<option value="'.$globals['permalink'].'/related" '.$active[8].'>'._('похожие')."</option>";
    $html .= '</select></div>';
    echo $html;
}
function do_comment_pages($total, $current, $reverse = true)
{
    ...
    for ($i = 1; $i <= $total_pages; $i++) {
        ...
        if ($total_pages < 7 || abs($i - $current) < 1 || $i < 3 || abs($i - $total_pages) < 2) {
            echo '<a href="'.get_comment_page_url($i, $total_pages, $query, $reverse).'" title="'._('перейти на страницу')." $i".'">'.$i.'</a>';
            continue;
        }
    ...
    if ($res) {
        ...
        foreach ($res as $comment) {
            ...
            if (
                !$self
                && $obj->vote < 0
                && $link->negatives < $link->votes * 0.5// Dont show negative comment if already has many
...
function do_qanda_text($link)
{
    ...
    $qas = get_qanda($link);
    do_header(_('Q&A вопросы').": $link->title", 'post');
    foreach ($qas as $qa) {
        $a = $qa->answer;
