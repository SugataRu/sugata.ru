...
if(!empty($_REQUEST['time'])) {
	...
	$last_modified = time();
	$title = $globals['site_name'].': '.sprintf(_('наиболее проголосовали в %s'), txt_time_diff($from));
	...
	$title = $globals['site_name'].': '.sprintf(_('фавориты %s'), $user_login);
	...
	$user_login = $db->get_var("select user_login from users where user_id=$user_id");
	$title = $globals['site_name'].': '.sprintf(_('проголосовал за %s'), $user_login);
	$globals['redirect_feedburner'] = false;
	...
	$user_login = $db->get_var("select user_login from users where user_id=$user_id");
	$title = $globals['site_name'].': '.sprintf(_('Друзья %s'), $user_login);
	$globals['redirect_feedburner'] = false;
	...
	$user_login = $db->get_var("select user_login from users where user_id=$user_id");
	$title = $globals['site_name'].': '.sprintf(_('новости от %s'), $user_login);
	$globals['redirect_feedburner'] = false;
} elseif (isset($_REQUEST['active'])) {
	$globals['redirect_feedburner'] = false;
	$title = $globals['site_name'].': '._('более активный');
	...
	*/
	...
	switch ($status) {
		case 'published':
			$order_field = 'date';
			$link_date = 'date';
			$title = $globals['site_name'].': '._('опубликованный');
			break;
		case 'queued':
			$title = $globals['site_name'].': '._('в очереди');
			$order_field = 'date';
			$link_date = "date";
			$home = "/queue";
...
if ($links) {
	foreach($links as $link_id) {
		...
		// sugata own namespace
		echo "		<sugata:link_id>$link->id</sugata:link_id>\n";
		echo "		<sugata:sub>$link->sub_name</sugata:sub>\n";
		echo "		<sugata:status>$link->status</sugata:status>\n";
		echo "		<sugata:user>$link->username</sugata:user>\n";
		echo "		<sugata:clicks>".$link->clicks."</sugata:clicks>\n";
		echo "		<sugata:votes>".intval($link->votes+$link->anonymous)."</sugata:votes>\n";
		echo "		<sugata:negatives>$link->negatives</sugata:negatives>\n";
		echo "		<sugata:karma>".intval($link->karma)."</sugata:karma>\n";
		echo "		<sugata:comments>$link->comments</sugata:comments>\n";
		echo "		<sugata:url>".htmlspecialchars($link->url)."</sugata:url>\n";
		...
		if (isset($_REQUEST['nohtml'])) {
			...
			echo '<p>'.$content.'</p>';
			echo '<p><strong>' . _('метки') . '</strong>: ' . preg_replace('/,([^ ])/', ', $1', $link->tags) . '</p>';

			if ($link->status != 'published') $rel = 'rel="nofollow"';
			else $rel = '';

			echo '<p>&#187;&nbsp;<a href="http://'.get_server_name().$globals['base_url'].'go?id='.$link->id.'"';
			echo " $rel>"._('оригинальные новости')."</a> (".parse_url($link->url, 1).")</p>";
...
function do_header($title) {
	...
	echo '	xmlns:sugata="https://sugata.ru/faq-es.php"'."\n";
	echo ' >'. "\n";
	echo '<channel>'."\n";
	...
	echo"	<image><title>$title</title><link>http://".get_server_name().$home."</link><url>http://".get_static_server_name().$globals['base_url']."img/eli-rss.png</url></image>\n";
	echo'	<description>'._('Совместный сайт для публикации и общения ').'</description>'."\n";
	echo'	<pubDate>'.date("r", $last_modified).'</pubDate>'."\n";
	echo'	<generator>httpы://sugata.ru/</generator>'."\n";
...
function check_redirect_to_feedburner($status) {
	...
	$regex = '/'.$globals['rss_redirect_user_agent'].'|pubsub|sugata|burner/i';
