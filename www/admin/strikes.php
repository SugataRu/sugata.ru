...
function do_strike_new($selected_tab)
{
    ...
    if ($strike->getUserCurrentStrike()) {
        $error = 'У этого пользователя уже есть нарушение, еще не законченная';
    } elseif ($user->level === 'disabled') {
        $error = 'Этот пользователь в настоящее время заблокирован';
    } else {
        $error = null;
    }
...
function do_strike_save()
{
    ...
    if (!$user->read()) {
        die('Несуществующий пользователь');
    }
