...
if ($_REQUEST['q']) {
    ...
    $title = _('Заметьте меня').': '._('поиск в заметках') . ': ' . htmlspecialchars(strip_tags($_REQUEST['q']));
    ...
    $title = _('Заметьте меня').': '.sprintf(_('записки от %s'), $username);
    ...
    $title = _('Заметьте меня').': '.sprintf(_('друзья  %s'), $username);
    ...
    $title = _('Заметьте меня').': '.sprintf(_('разговор о %s'), $username);
} else {
    //
    // All posts
    //
    ...
    $title = _('Заметьте меня').': '._('примечания');
}
...
function do_header($title)
{
    ...
    echo '	<generator>http://blog.sugata.ru/</generator>'."\n";
