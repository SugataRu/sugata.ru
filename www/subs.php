...
if (empty($routes)) {
    die; // Не позволяйте вызывать в обход диспетчера
}

$globals['description'] = 'Пространства на Sugata. Они служат для классификации информации по отдельным потокам. Создавайте пространства и ведите их.';
...
if (!empty($_GET['q'])) {
    $q = trim(preg_replace('/[^a-zA-Zа-яА-Я0-9\s]/', '', strip_tags($_GET['q'])));
} else {
    $q = null;
}
...
do_header(_("Пространства "), 'Пространства');
...
switch ($option) {
    ...
    default:
        ...
        if (!$q && !empty($_GET['c'])) {
            $char_selected = preg_replace('/[^A-ZА-Я]/', '', substr($_GET['c'], 0, 1));
        }
...
$can_edit = (SitesMgr::my_id() == 4 && SitesMgr::can_edit(0));
