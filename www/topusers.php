...
$items = array(_('участник'),  _('карма'), _('новости'), _('постов'), _('комментариев'), _('голоса за последние 2 месяца'));
...
do_header(_('Участники') . ' | ' . $globals['site_name']);

echo '<div id="singlewrap"  style="padding: 0 25px 25px 25px;">' . "\n";
echo '<div class="topheading"><h1>'._('Статистика').'</h1></div>';
...
if ($users) {
    foreach ($users as $dbuser) {
        ...
        $user->print_medals();
        echo '</td>';
        echo '<td class="short">2'.$user->total_links.'</td>';
