...
if (! empty($_REQUEST['id']) && intval($_REQUEST['id']) > 0) {
    ...
    if (!empty($_REQUEST['time'])) {
        ...
        $title = $site_info->name.': '.sprintf(_('наиболее проголосовали в %s'), txt_time_diff($from));
        ...
        $title = $site_info->name.': '.sprintf(_('любимые в %s'), $user_login);
        ...
        $user_login = $db->get_var("select user_login from users where user_id=$user_id");
        $title = $site_info->name.': '.sprintf(_('проголосовал за %s'), $user_login);
        ...
        $user_login = $db->get_var("select user_login from users where user_id=$user_id");
        $title = $site_info->name.': '.sprintf(_('Друзья %s'), $user_login);
        ...
        $user_login = $db->get_var("select user_login from users where user_id=$user_id");
        $title = $site_info->name.': '.sprintf(_('новости от %s'), $user_login);
    } elseif (isset($_REQUEST['active'])) {
        $title = $site_info->name.': '._('более активный');
        ...
        switch ($status) {
            case 'published':
                $order_field = 'date';
                $link_date = 'date';
                $title = $site_info->name.': '._('опубликованный');
                break;
            case 'queued':
                $title = $site_info->name.': '._('в очереди');
                $order_field = 'date';
                $link_date = "date";
                $home = "/queue";
                break;
