...
//print_r('0<br><br>0');
//print_r(SitesMgr::can_edit('1'));


if (! $can_edit) {
    $error = _("Вы не можете редактировать и создавать новые");
} elseif ($_POST['created_from']) {
    try {
        $sub = SitesMgr::get_info(save_sub($id), true);

        die(header("Location: ".$globals['base_url_general']."s/$sub->name/subedit"));
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
...
function save_sub($id)
{
    ...
    if (!SitesMgr::can_edit($id)) {
        throw new Exception(_('У вашего пользователя нет прав на редактирование'));
    }
    ...
    if ($_POST['created_from'] != $site->id) {
        throw new Exception(_('Сайт не верный формат'));
    }

    if ($owner != $current_user->user_id && ! $current_user->admin) {
        throw new Exception(_('Имя владельца не корректно'));
    }

    $name = mb_substr(clean_input_string($_POST['name']), 0, 12);

    if (mb_strlen($name) < 3 || !preg_match('/^\p{L}[\p{L}\d_]+$/u', $name)) {
        throw new Exception(_('Имя неверное'). ' ' . $_POST['name']);
    }

    $name_long = mb_substr(clean_text($_POST['name_long']), 0, 40);

    if (mb_strlen($name_long) < 3) {
        throw new Exception(_('Название неверно'));
    }
    ...
    if ($db->get_var("select count(*) from subs where name = '$name' and id != $id") > 0) {
        throw new Exception(_('Уже существует еще один подзаголовок с именем').' <strong>'.$name.'</strong>');
    }
    ...
    if (empty($r) || empty($id)) {
        throw new Exception(_('Произошла ошибка обновления базы данных'));
    }
