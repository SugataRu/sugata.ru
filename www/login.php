...
function do_login()
{
    ...
    if (empty($error) && (strpos($_REQUEST['return'], '/submit') !== false)) {
        $info = _('Для отправки истории вы должны быть зарегистрированным участником');
    } else {
        $info = null;
    }
...
function do_login_post()
{
    ...
    if (($failed > 2 || ($globals['captcha_first_login'] && ! UserAuth::user_cookie_data())) && !ts_is_human()) {
        ...
        return array(_('неверный код безопасности'), $failed);
    }
    ...
    if (strlen($password) > 0 && !$current_user->Authenticate($username, $password, $_POST['persistent'])) {
        ...
        return array(_('пользователь или e-mail не существует, или неверный пароль'), $failed);
    }
...
function do_recover_post()
{
    ...
    if (!ts_is_human()) {
        return _('неверный код безопасности');
    }

    $user = new User();

    if (!preg_match('/.+@.+\..+$/', $_POST['email'])) {
        return _('адрес e-mail не действителен');
    }

    $user->email = $_POST['email'];

    if (!$user->read()) {
        return _('e-mail не связан ни с одной учетной записью');
    }

    if ($user->disabled()) {
        return _('учетная запись отключена');
    }

    require_once(mnminclude.'mail.php');

    if (!send_recover_mail($user, false)) {
        return _('Невозможно отправить письмо для восстановления пароля');
    }
