...
$page_title = _('Заметки');

$globals['description'] = 'Заметки на Sugata. Это отличное место, чтобы вы были замечены. Пишите короткие сообщения и люди пойдут за вами. Делитесь интересными историями.';
...
switch ($argv[0]) {
    case '_best':
        $tab_option = 2;
        $page_title = _('Лушие заметки') . ' | ' . _('sugata');
...
if ($tab_option == 4) {
    ...
    $options = array(
        ...
        _('люди') => $user->get_uri('notes_friends'),
        _('любимое') => $user->get_uri('notes_favorites'),
        _('разговор') . $conversation_extra => $user->get_uri('notes_conversation'),
        _('голоса') => $user->get_uri('notes_votes'),
        sprintf(_('дебаты с %s'), $user->username) => $globals['base_url'] . "between?type=posts&amp;u1=$current_user->user_login&amp;u2=$user->username",
        sprintf(_('профиль %s'), $user->username) => $user->get_uri(),
    ...
    $options = array(
        _('все') => post_get_base_url(''),
        _('люди') => $current_user->get_uri('notes_friends'),
        _('любимое') => $current_user->get_uri('notes_favorites'),
        _('разговор') . $conversation_extra => $current_user->get_uri('notes_conversation'),
        _('голоса') => $current_user->get_uri('notes_votes'),
        _('последние фото') => "javascript:fancybox_gallery('post');",
        _('дебаты') . '&nbsp;&rarr;' => $globals['base_url'] . "between?type=posts&amp;u1=$current_user->user_login",
    );
} else {
    $options = false;
}

do_header($page_title, _('Заметки'), get_posts_menu($tab_option, $user->username), array($options, $view, $rss_option));
