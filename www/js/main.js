...
function updateVote($voted, $notvoted, id, data) {
    if (data.error) {
        return mDialog.notify("{% trans _('Хм:') %} " + data.error, 5);
    }
...
function parseLinkAnswer(id, link) {
    ...
    if (link.error || id != link.id) {
        disable_vote_link(id, -1, "{% trans _('опсс') %}", '');
        mDialog.notify("{% trans _('Хм:') %} " + link.error, 5);
        return false;
    }
...
function checkField($input, $button) {
    var name = $input.attr('id'),
        value = $input.val(),
        url = base_url + 'backend/checkfield.php?name=' + name + '&value=' + encodeURIComponent(value);
...
function report_problem(frm, user, id) {
    ...
    mDialog.confirm(
        "{% trans _('Вы уверены, что это ') %} <em>" + frm.ratings.options[frm.ratings.selectedIndex].text + "</em>?",
        function() {
...
function change_fav_readed(element, type, id) {
    ...
    $.post(base_url + 'backend/get_favorite_readed', {
        ...
        if (data.error) {
            mDialog.notify("{% trans _('Хм:') %} " + data.error, 5);
            return;
        }
...
var mDialog = new function() {
    ...
    this.init = function() {
        ...
        this.btYes = $("<button>").attr({ id: 'mDialog_yes' }).text("{% trans _('Да') %}");
        this.btNo = $("<button>").attr({ id: 'mDialog_no' }).text("{% trans _('нет') %}");
        this.btOk = $("<button>").attr({ id: 'mDialog_ok' }).text("{% trans _('Хорошо') %}");
        this.btCancel = $("<button>").attr({ id: 'mDialog_ok' }).text("{% trans _('Отменить') %}");
...
function show_report_dialog(comment_id) {
    ...
    $.colorbox({
        ...
        opacity: 0.1,
        transition: 'none',
        title: "{% trans _('жалоба на комментарий') %}",
        scrolling: false,
        open: true,
        className: 'report',
        width: w,
...
var fancyBox = new function() {
    this.parse = function($e) {
        ...
        if ((v = myHref.match(/(?:youtube\.com\/(?:embed\/|.*v=)|youtu\.be\/)([\w\-_]+).*?(#.+)*$/))) {
            ...
            iframe = true;
            title = '<a target="_blank" rel="nofollow" href="' + myHref + '"' + target + '>{% trans _('Видео на Youtube') %}</a>';
...
(function() {
    ...
    function connect() {
        ...
        $.getJSON(base_url + "backend/notifications.json.php?check=" + check_counter + "&has_focus=" + has_focus, function(data) {
            var now = new Date().getTime();
