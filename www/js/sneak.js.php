...
function received_data(data) {
    ...
    if (requests > max_requests) {
        ...
        total_requests = 0;
        next_update = 100;
        mDialog.confirm('<?php echo _('Вы хотите продолжить диалог?');?>', do_play, do_pause);
        return;
    }
...
function send_chat(form) {
    ...
    if(!is_playing()) {
        mDialog.notify("<?php echo _('пауза'); ?>", 3);
        return false;
    }
    if(global_options.show_chat == false) {
        mDialog.notify("<?php echo _('у вас отключена обратная связь'); ?>", 3);
        return false;
    }
    if(!form.comment.value.match(/^!/) && form.comment.value.length < 4) {
        mDialog.notify("<?php echo _('слишком короткое сообщение'); ?>", 3);
        return false;
    }
    if( currentTime.getTime() < last_comment_sent + (comment_period*1000)) {
        mDialog.notify("<?php echo _('вы можете отправить только одно сообщение за каждый ');?> " + comment_period + " <?php echo _(' переод');?>", 3);
        return false;
    }
    abort_request();
