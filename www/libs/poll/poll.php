...
class Poll
{
        ...
        foreach ($options as $data) {
            ...
            if ($id) {
                if (empty($this->options[$id])) {
                    ...
                    throw new Exception(_('Параметры опроса недействительны'));
                }
    ...
    public function validateQuestion()
    {
        ...
        if (mb_strlen($this->question) > $globals['polls_question_len_limit']) {
            throw new Exception(sprintf(_('Ограничение длины вопроса %s символов'), $globals['polls_question_len_limit']));
        }
    }
    ...
    public function validateOptions()
    {
        ...
        if (($count < 2) || ($count > $this->options_limit)) {
            throw new Exception(sprintf(_('Вы должны указать минимум  %s параметров и максимум %s'), 2, $this->options_limit));
        }
        ...
        foreach ($this->getOptions() as $option) {
            if (mb_strlen($option->option) > $globals['polls_option_len_limit']) {
                throw new Exception(sprintf(_('Ограничение длины на опцию %s символов'), $globals['polls_option_len_limit']));
            }

            if (in_array($option->option, $duplicated)) {
                throw new Exception(_('Варианты опроса дублируются'));
            }
    ...
    public function validateDuration($hours)
    {
        if (!in_array($hours, $this->durations_valid)) {
            throw new Exception(_('Продолжительность указанная в опросе недействительна'));
        }
    }
