...
class Link extends LCPBase
{
    ...
    public function getTimePublishedAsString()
    {
        ...
        if (($interval->d >= 3) || (($interval->d > 1) && ($interval->h === 0))) {
            return sprintf(_(' %s дн.'), $interval->d);
        }

        if ($interval->d > 1) {
            return sprintf(_(' %s дн. %s час.'), $interval->d, $interval->h);
        }
        ...
        if ($interval->d === 1) {
            return sprintf(_(' 1 день  %s час.'), $interval->h);
        }

        if (($interval->h > 1) && ($interval->i === 0)) {
            return sprintf(_(' %s час.'), $interval->h);
        }

        if ($interval->h > 1) {
            return sprintf(_(' %s час. %s мин.'), $interval->h, $interval->i);
        }
        ...
        if ($interval->h === 1) {
            return sprintf(_(' 1 час. %s мин.'), $interval->i);
        }

        if ($interval->i > 1) {
            return sprintf(_(' %s мин.'), $interval->i);
        }
        ...
        return sprintf(_(' %s сек.'), $interval->s);
    }
    ...
    public function json_votes_info($value = false)
    {
        ...
        if ($value) {
            ...
            if ($value < 0) {
                ...
                $dict['vote_description'] = _('Готово!');
            }
        }
    ...
    public function check_url($url, $check_ban = true, $first_level = false)
    {
        ...
        if (preg_match("/^$quoted_domain$/", $url_components['host'])) {
            $this->ban = array();
            $this->ban['comment'] = _('сервер локальный');

            syslog(LOG_NOTICE, "Sugata, server name is local name ($current_user->user_login): $url");
        ...
        syslog(LOG_NOTICE, "Sugata, server name is banned ($current_user->user_login): $url");
    ...
    public function get($url, $maxlen = 150000, $check_ban = true)
    {
        ...
        if (($response = get_url($url))) {
            ...
            if (!empty($new_url) && $new_url !== $url) {
                syslog(LOG_NOTICE, "Sugata, redirected ($current_user->user_login): $url -> $new_url");
                ...
                if (strlen($new_url) < 300 && ($new_url_components = @parse_url($new_url))) {
                    if ($url_components['host'] !== $new_url_components['host']) {
                        syslog(LOG_NOTICE, "Sugata, changed source URL ($current_user->user_login): $url -> $new_url");
            ...
            syslog(LOG_NOTICE, "Sugata, error getting ($current_user->user_login): $url");
        ...
        if (preg_match('/<!-- *noshare *-->/', $this->html)) {
            $this->ban = array();
            $this->ban['comment'] = _('автор не хочет, чтобы статья отправлялась, надо уважать его пожелания');

            syslog(LOG_NOTICE, "Sugata, noshare ($current_user->user_login): $url");
    ...
    public function create_blog_entry()
    {
        ...
        if (!$blog->read('key') || ($blog->type !== 'noiframe' && $this->noiframe)) {
            if ($blog->type !== 'noiframe' && $this->noiframe) {
                $blog->type = 'noiframe';
                syslog(LOG_INFO, "Sugata, changed to noiframe ($blog->id, $blog->url)");
            }
    ...
    public function print_summary(
		...
		if ($this->url_str == 'youtube.com') { 
		
		preg_match('#(\.be/|/embed/|/v/|/watch\?v=)([A-Za-z0-9_-]{5,11})#', $this->url, $matches);
       if(isset($matches[2]) && $matches[2] != ''){
          $this->id_youtube = $matches[2];
        }
		
		//$this->url_str2 = '111111111111111'.$this->url;
		}
    ...
    public function get_canonical_permalink($page = false)
    {
        ...
        return $globals['scheme'].'//sugata.ru'.$this->get_relative_permalink(true).$page;
    }
    ...
    public function get_status_text($status = false)
    {
        ...
        switch ($status) {
            case 'abuse':
                return _('нарушения (невидим)');

            case 'discard':
                return _('отброшенный');
    ...
    public function calculate_karma()
    {
        ...
        if ($n > $votes_pos / 5) {
            $this->annotation .= intval($n / $votes_pos * 100)._('% голосов с высоким сходством')."<br/>";
        }
        ...
        if ($karma_pos_user_high == 0 || $karma_pos_user_low / $karma_pos_user_high > 1.15) {
            ...
            $this->annotation .= $perc._('% голосов с кармой ниже среднего')
        ...
        if ($this->coef < .99) {
            $this->annotation .= _('Новости «старые»')."<br/>";
        } elseif ($this->coef > 1.01) {
            $this->annotation .= _('Бонус за последние новости')."<br/>";
        }
        ...
        if ($meta_coef && $meta_coef[$this->sub_id]) {
            ...
            if (abs(1 - $meta_coef[$this->sub_id]) > 0.05) {
                $this->annotation .= _('Коэффициент пространства')
        ...
        if (
            ...
            $this->annotation .= _('Бонусные клики').": $bonus<br/>";
            $this->karma += $bonus;
        }
    ...
    public function get_thumb($debug = false, $url = false)
    {
        ...
        if ($debug) {
            echo "<!-- Sugata, before image_parser -->\n";
        }

        $img = $this->image_parser->get();

        if ($debug) {
            echo "<!-- Sugata, after image_parser: $img->url -->\n";
        }
        ...
        if ($img) {
            ...
            if ($this->move_tmp_image(basename($filepath), 'image/jpeg')) {
                ...
                if ($debug) {
                    echo "<!-- Sugata, new thumbnail $img->url -->\n";
                }
            } else {
                $this->thumb_status = 'error';

                if ($debug) {
                    echo "<!-- Sugata, error saving thumbnail ".$this->get_permalink()." -->\n";
                }
            }
        }
    ...
    public function check_field_errors()
    {
        ...
        if (!$this->sub_id && ($this->content_type !== 'article')) {
            return _('Вы не выбрали пространство');
        }
        ...
        if ($this->sub_id) {
            ...
            if (empty($this->url) && empty($properties['no_link'])) {
                return _('В этом пространстве обязательно отправлять содержимое по ссылке');
            }
        }
        ...
        if (mb_strlen($title) < 8) {
            return __('Название слишком короткое, должно быть не менее %s символов', 8);
        }

        if (mb_strlen($title) > 120) {
            return __('Название слишком длинное, оно должно быть не более %s символов', 120);
        }

        if (get_uppercase_ratio($title) > 0.5) {
            return _('В названии слишком много столиц');
        }
        ...
        if (
            ...
            return __('Текст слишком короткий, он должен быть более %s символов', $properties['intro_min_len']);
        }

        if (($this->content_type !== 'article') && get_uppercase_ratio($content) > 0.3) {
            return _('В тексте слишком много букв');
        }

        if (strlen($this->tags) < 3) {
            return _('Вы не ставили теги');
        }

        if (preg_match('/.*http:\//', $title)) {
            return _('Вы не можете добавлять ссылки в заголовке');
        }
    }
    ...
    public function delete()
    {
        ...
        if (
            ...
            throw new Exception(_('Невозможно удалить это'));
        }
