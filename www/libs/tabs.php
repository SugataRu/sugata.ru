...
final class Tabs
{
    ...
    public static function renderForIndex($option, $tab_class)
    {
        ...
        $items = array();
        $items[] = array('id' => 0, 'url' => $globals['meta_skip'], 'title' => _('все'));

        if (isset($current_user->has_subs)) {
            $items[] = array('id' => 7, 'url' => $globals['meta_subs'], 'title' => _('подписки'));
        }
        ...
        switch ($option) {
            case 7: // Personalised, published
                $feed = array("url" => "?subs=" . $current_user->user_id, "title" => _('подписки'));
                break;
        ...
        if ($current_user->user_id > 0) {
            $items[] = array('id' => 1, 'url' => '?meta=_friends', 'title' => _('люди'));
        }
    ...
    public static function renderForShakeIt($option = -1, $tab_class)
    {
        ...
        $items = array();
        $items[] = array('id' => 1, 'url' => 'queue' . $globals['meta_skip'], 'title' => _('все'));

        if ($current_user->has_subs) {
            $items[] = array('id' => 7, 'url' => 'queue' . $globals['meta_subs'], 'title' => _('подписки'));
        }
        ...
        $items[] = array('id' => 3, 'url' => 'queue?meta=_popular', 'title' => _('кандидаты'));

        if ($current_user->user_id > 0) {
            $items[] = array('id' => 2, 'url' => 'queue?meta=_friends', 'title' => _('люди'));
        }

        if (!$globals['bot']) {
            $items [] = array('id' => 5, 'url' => 'queue?meta=_discarded', 'title' => _('отбросить'));
        }
    ...
    public static function renderForSneakme($options, $tab_class)
    {
        ...
        $html .= '<li class="icon wideonly"><a href="' . $globals['base_url'] . $rss . '" title="' . $rss_title . '"><i class="lnr lnr-frame-contract"></i> RSS</a></li>';
        $html .= '</ul>' . "\n";
    ...
    {
        return self::renderUserProfileSubheader(self::optionsForProfileProfile(), 0, 'rss?friends_of=' . $user->id, _('рассылки от друзей в rss2'), $tab_class);
    }
    ...
    public static function optionsForProfileArticles()
    {
        ...
        if ($discard = Link::userArticlesDraft($user)) {
            $discard = __('Черновиков (%s)', $discard);
        } else {
            $discard = __('Черновики');
        }
    ...
    {
        return self::renderUserProfileSubheader(self::optionsForProfileFriends(), 0, 'rss?friends_of=' . $user->id, _('рассылки от друзей в rss2'), $tab_class);
    }
    ...
    {
        return self::renderUserProfileSubheader(self::optionsForProfileHistory(), 0, 'rss?sent_by=' . $user->id, _('рассылка  rss2'), $tab_class);
    }
    ...
    {
        return self::renderUserProfileSubheader(self::optionsForProfileShaken(), 1, 'rss?voted_by=' . $user->id, _('проголосовал в rss2'), $tab_class);
    }
    ...
    {
        return self::renderUserProfileSubheader(self::optionsForProfileFavorites(), 2, 'rss?voted_by=' . $user->id, _('проголосовал в rss2'), $tab_class);
    }
    ...
    {
        return self::renderUserProfileSubheader(self::optionsForProfileCommented(), 0, 'comments_rss?user_id=' . $user->id, _('комментарии в rss2'), $tab_class);
    }
    ...
    public static function optionsForProfileCommented()
    {
        ...
        return array(
            ...
            'conversation' => array(
                'title' => _('Разговор').$globals['extra_comment_conversation'],
                'link' => $user->get_uri('conversation'),
            ),
            'shaken_comments' => array(
                'title' => _('Проголосовать'),
