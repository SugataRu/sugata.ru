...
// Следующие функции относятся к пользователям, но не выполняются как класс, поэтому их легко использовать с User и UserAuth.
define('FRIEND_YES', '<img src="'.$globals['base_static'].'img/common/icon_friend_00.png" alt="del" width="18" height="16" title="'._('друг').'"/>');
define('FRIEND_BOTH', '<img src="'.$globals['base_static'].'img/common/icon_friend_bi_00.png" alt="del" width="18" height="16" title="'._('пользователи').'"/>');
define('FRIEND_NO', '<img src="'.$globals['base_static'].'img/common/icon_friend_no_00.png" alt="add" width="18" height="16" title="'._('добавить список друзей').'"/>');
define('FRIEND_OTHER', '<img src="'.$globals['base_static'].'img/common/icon_friend_other_00.png" alt="add" width="18" height="16" title="'._('избран').'"/>');
define('FRIEND_IGNORE', '<img src="'.$globals['base_static'].'img/common/icon_friend_ignore_00.png" alt="add" width="18" height="16" title="'._('игнорируемый').'"/>');
...
class User
{
    ...
    public static function get_menu_items($view, $user)
    {
        ...
        if ($user->id == $current_user->user_id) {
          //  $items[] = new MenuOption(_('Написать историю'), $globals['base_url'] . 'submit?type=article&write=true', $id, _('отправить новую историю'), 'submit_new_post');
        }

        $items[] = new MenuOption(_('профиль'), $user->get_uri('profile'), $id, _('Информация о пользователе'));
        $items[] = new MenuOption(_('история'), $user->get_uri('history'), $id, _('Истории мои'));
        $items[] = new MenuOption(_('пространство'), $user->get_uri('subs'), $id, _('Пространства Sugata'));
        $items[] = new MenuOption(_('комментарии'), $user->get_uri('commented'), $id, _('Информация обратной связи'));
        $items[] = new MenuOption(_('примечания'), $user->get_uri('notes'), $id, _('Страница заметок'));
        $items[] = new MenuOption(_('отношения'), $user->get_uri('friends'), $id, _('Друзья и игнорируемые'));

        if ($user->id == $current_user->user_id) {
            $items[] = new MenuOption(_('частный'), $user->get_uri('notes_privates'), $id, _('Частные заметки'));
        }
...
// END meta
    ...
    private function getMedalRatio()
    {
        ...
        return array(
            'type' => $type,
            'title' => __('опубликованный процент (%s)', $ratio)
        );
    }
    ...
    private function getMedalPublished()
    {
        ...
        return array(
            'type' => $type,
            'title' => __('публикации (%s)', $this->published_links)
        );
    }
    ...
    private function getMedalFriends()
    {
        ...
        return array(
            'type' => $type,
            'title' => __('друзей (%s)', $this->total_friends)
        );
    }
    ...
    public function add_karma($inc, $log = false)
    {
        ...
        if (!empty($log) && mb_strlen($log) > 5) {
            $this->karma_log .= "$log: $inc, " . _('новая карма') . ": $this->karma\n";
