...
class Comment extends LCPBase
{
    ...
    public function store($full = true)
    {
        ...
        if ($this->id === 0) {
			...
			$mobile = intval($globals['mobile']);  
			
            $r = $db->query("INSERT INTO comments (comment_user_id, comment_link_id, comment_type, comment_karma, comment_ip_int, comment_ip, comment_date, comment_randkey, comment_content, comment_order,mobile) VALUES ($this->author, $this->link, '$comment_type', $this->karma, $this->ip_int, '$this->ip', FROM_UNIXTIME($this->date), $this->randkey, '$comment_content', $this->order, $mobile)");
    ...
    public static function print_form($link, $rows = 5)
    {
       ...
       // if (!$link->comments_allowed()) {
		if (!$link || !$link->comments_allowed()) {
            ...
            echo '<fieldset>'."\n";
            echo '<legend>'._('отправить комментарий').'</legend>';
            $vars = compact('link', 'comment');
            Haanga::Load('comment_edit.html', $vars);
            echo '<div class="note" style="margin-top:10px">'._('спам, нападки, нарушение законодательства приведут к ануллированию аккаунта...').'</div>';
            echo '</fieldset>'."\n";
            echo '</form>'."\n";
            echo "</div>\n";
        } else {
            ...
            if ($current_user->authenticated) {
                if ($current_user->user_date >= $globals['now'] - $globals['min_time_for_comments']) {
                    ...
                    $msg = _('вы должны подождать')." $remaining "._('чтобы написать первый комментарий');
                }
                if ($current_user->user_karma < $globals['min_karma_for_comments']) {
                    $msg = _('у вас нет минимальной требуемой кармы')." (".$globals['min_karma_for_comments'].") "._('комментировать').": ".$current_user->user_karma;
                }
                echo '<div class="commentform warn">'."\n";
                echo $msg."\n";
                echo '</div>'."\n";
                ...
                echo '<div class="commentform warn">'."\n";
                echo '<a href="'.get_auth_link().'login.php?return='.urlencode($globals['uri']).'">'._('Войдите').'</a> '._('или').' <a href="'.$globals['base_url'].'register.php">зарегистрируйтесь.</a>'."\n";
                echo '</div>'."\n";
                print_oauth_icons();
            }
        }
    ...
    public static function save_from_post($link, $redirect = true)
    {
        ...
        if ($hours > 0) {
            ...
            if ($clones) {
                ...
                if ($c > 0) {
                    ...
                    return _('вы уже прокомментировали');
                }
            }
        }
        ...
        if (!$current_user->admin && $current_user->user_karma < 6.2) {
            ...
            if ($link->status !== 'published' && $link->ip == $globals['user_ip'] && $link->author != $comment->author) {
                ...
                return _('вы уже комментировали это');
            }
            ...
            if (intval($db->get_var("select count(*) from comments where comment_link_id = $link->id and comment_ip='$comment->ip' and comment_user_id != $comment->author")) > 1) {
                ...
                return _('слишком много комментариев от вас');
            }
        }
        if (mb_strlen($comment->content) < 5 || !preg_match('/[a-zа-яА-ЯA-Z:-]/', $_POST['comment_content'])) {
            // Check there are at least a valid char
            return _('очень короткий текст или недопустимые символы');
        }
        if (!$current_user->admin && ($link->author != $comment->author)) {
            ...
            if ($comment->banned && $current_user->Date() > $globals['now'] - 86400) {
                ...
                return _('комментарий не вставлен, ссылка на сайт отключена (и недавний пользователь)');
            }
        ...
        if ($stored) {
            $db->rollback();
            return _('дублированный комментарий');
        }
        ...
        if ($karma_penalty > 0) {
            ...
            $user->add_karma(-$karma_penalty, _('повторный текст или злоупотребление ссылками в комментариях'));
            return _('штраф за карму за повторный текст или злоупотребление ссылками');
        }
        if (!$comment->store()) {
            $db->rollback();
            return _('ошибка вставки комментария');
        }
        $comment->insert_vote();
