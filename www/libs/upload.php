...
class Upload
{
    ...
    public function create_thumbs($key = false)
    {
        ...
        if (!$thumb->load($pathname)) {
            ...
            syslog(LOG_INFO, "Sugata, попробуйте альтернативу превью ($alternate_image) for $pathname");
    ...
    public function from_temporal($file, $type = false)
    {
        ...
        if (Upload::user_uploads($current_user->user_id, 24) > $globals['media_max_upload_per_day']) {
            return _('превышен максимальный дневной предел');
        }
        if (Upload::user_bytes_uploaded($current_user->user_id, 24) > $globals['media_max_bytes_per_day'] * 1.2) {
            return _('превышено максимальное количество байт в день');
        }
        if ($type && ! preg_match("/^$type\/[^ ]+$/", $file['type'])) {
            return _('неверный тип');
        }
        ...
        if (!@move_uploaded_file($file['tmp_name'], $this->pathname())) {
            syslog(LOG_INFO, "Sugata, ошибка при перемещении в ".$this->pathname());
            return false;
        }
    ...
    public function check_size_and_rotation($pathname)
    {
        ...
        if (($pathname !== $original) && file_exists($pathname) && !@rename($pathname, $original)) {
            syslog(LOG_INFO, "Ошибка переименования файла $pathname -> $original");
            @unlink($pathname);
        }
